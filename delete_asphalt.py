...
def local_ransac(points: List[LidarPoint], num_iterations: int, distance_threshold: float, min_plane_points: int) -> List[LidarPoint]:
    """Локальный RANSAC для фильтрации точек асфальта."""
    best_plane = None
    best_inliers = []
...
def grid_segmentation(points: List[LidarPoint], grid_size: float) -> dict:
    """Разбивает точки на прямоугольные сегменты (grid-based segmentation)."""
    segments = {}
...
def filter_asphalt(points: List[LidarPoint]) -> List[LidarPoint]:
    ...
    GRID_SIZE = 7.50  # Размер ячейки сетки в метрах
    ...
    # Фильтрация точек в каждом сегменте
    filtered_points = []
